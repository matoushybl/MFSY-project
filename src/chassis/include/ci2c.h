...
#ifdef LOG
            printf("I2C: %2x WRITE[%d] ", chip_addr, nw);
        for (int i=0; i<nw;i++)
          printf("%2x ", buffer[i]);
...
#ifdef LOG
            printf("I2C: %2x READ[%d] ", chip_addr, nr);
        for (int i=0; i<nr;i++)
          printf("%2x ", buffer[i]);
...
// array reads
    ...
    static inline int
    i2c_rmw_leuint16(int fd, int chip_addr, int reg, uint16_t clearbits, uint16_t setbits, uint16_t togglebits) {
        uint16_t value;
        int err = i2c_read_leuint16(fd, chip_addr, reg, &value);
        if (err < 0)
            return err;

        return i2c_write_leuint16(fd, chip_addr, reg, ((value & ~clearbits) | setbits) ^ togglebits);
    }
